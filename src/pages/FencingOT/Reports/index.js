...
import DataTableContent from './Content/DataTable/index';
import HandsetHistoryContent from './Content/HandsetHistory';
import PivotTable from './Content/PivotTable';
...
import LinkTree from "./Content/LinkTree";
import MapToolsQuery from './Query/MapTools';
import ZoneDataMap from './Content/DataMap/ZoneDataMap';
import CompareGeoFence from './Content/DataMap/CompareGeoFence';
import ClusterMap from './Content/DataMap/ClusterMap';
import POIMapPreview from '../../CaseOT/POI/POIMapPreview';
import ZoneMapPreview from '../../CaseOT/Zones/ZoneMapPreview.js'
...
class Reports extends React.Component {
  constructor(props) {
    ...
    this.fetchCdrForLocationJob = this.fetchCdrForLocationJob.bind(this);
    this.fetchClusterData = this.fetchClusterData.bind(this);
  ...
  state = {
    ...
    newJob: {
      ...
      startTime: Math.round(
        new Date().setTime(new Date().getTime() - 7) / 1000
      ),
    ...
    //dataTableSelectedColumns: ['timestampdate', 'timestamptime', 'servedmsisdn', 'callingnumber','callednumber','locationlat','locationlon','callduration','locationnum'].map((colName) => (getCdrColumnName(colName))),
    allCdrColumns: [],
    mapEditToolsData: {
      spData: null,
    ...
    markers: [
      {
        ...
        position: {
          lat: 8.016038970947266,
          lng: 9.58817943397567,
        },
      },
    ],
    previewPane: [
  ...
  getMainContent() {
    switch (this.state.activeWidget) {
      case "Handset":
        return <HandsetHistoryContent selectedJob={this.state.selectedJob} />;
      case "Map":
        return (
          <ZoneDataMap
  ...
  getCdrDataTable() {
    return (
      <DataTableContent
        ...
        getRow={this.getRow}
        exportRight={this.state.currentAccount.modules.fenceot.export}
      />
    );
  }
      ...
      <div>
        ...
        >
          ...
          {leftDrawerOpen ? (
            ...
            >
              <ExpansionPanel
                expanded={this.state.caseSummaryExpanded}
                onChange={() =>
                  this.setState({
                    caseSummaryExpanded: !this.state.caseSummaryExpanded,
                  })
                }
                ltExpanded
              >
                ...
                <ExpansionPanelDetails>
                  <div>
                    ...
                    {this.state.selectedCase
                      ? this.state.selectedCase.name
                      : ""}
                    <br />

                    <strong>Description: </strong>
                    {this.state.selectedCase
                      ? this.state.selectedCase.description
                      : ""}
                    <br />

                    <strong>Category: </strong>
                    {this.state.selectedCase
                      ? this.state.selectedCase.category
                      : ""}
                    <br />
                    ...
                    {this.state.selectedCase ? (
                      <span
                        style={{
                          color:
                            this.state.selectedCase.status === "Open"
                              ? "green"
                              : "red",
                        }}
                      >
                    ...
                    <strong>Users: </strong>
                    {this.state.selectedCase.accounts
                      ? this.state.selectedCase.accounts.map(
                          (accountName, index) => (
                            <Chip
                              key={index}
                              label={accountName}
                    ...
                    {this.state.selectedCase.targets
              ...
              {this.state.currentAccount.modules.fenceot.add ? (
                <ExpansionPanel
                  defaultexpanded={this.state.jobsExpansionOpen}
                  onChange={(event, jobsExpansionOpen) =>
                    this.setState({ jobsExpansionOpen: !jobsExpansionOpen })
                  }
                  ltExpanded
                >
                  <ExpansionPanelSummary expandIcon={<ExpandMoreIcon />}>
                  ...
                  >
                    ...
                    <TextField
                      ...
                      label="Name"
                      value={
                        this.state.newJob.name ? this.state.newJob.name : ""
                      }
                      onChange={(event) =>
                        this.setState({
                          newJob: {
                    ...
                    <MuiPickersUtilsProvider utils={LuxonUtils}>
                      <KeyboardDateTimePicker
                        ...
                        value={new Date(this.state.newJob.startTime * 1000)}
                        onChange={(newDate) =>
                          this.setState({
                            newJob: {
                              ...this.state.newJob,
                              startTime: Math.round(newDate / 1000),
                            },
                          })
                        }
                      />
                    ...
                    <MuiPickersUtilsProvider utils={LuxonUtils}>
                      <KeyboardDateTimePicker
                        ...
                        value={new Date(this.state.newJob.endTime * 1000)}
                        onChange={(newDate) =>
                          this.setState({
                            newJob: {
                              ...this.state.newJob,
                              endTime: Math.round(newDate / 1000),
                            },
                          })
                        }
                      />
                    ...
                    {this.state.newJob.category === "IMSI" && (
                      ...
                      >
                        <InputLabel id="target-selector-label">
                          Target
                        </InputLabel>
                        <Select
                          ...
                          required
                        >
                          {this.props.selectedCase.targets
  ...
  async componentDidMount() {
    ...
    await this.getPOIs(selectedCase);
    await this.fetchZones(selectedCase);
  ...
  async onCreateJobButtonPress() {
    try {
      ...
      payload["case"] = this.props.selectedCase["id"];
      ...
      this.setState({
        alertType: "success",
        alertTitle: "New Job",
        alertMessage: "New Job Added Successfully",
        alertOpen: true,
      });
  ...
  async fetchCdrForJob(selectedJob) {
    ...
    this.setState({
      ...
      selectedJobCdrId: selectedJob.id,
      selectedJobCdrList: [],
    });
    ...
    try {
      ...
      if (currentPage < totalPages) {
        for (let i = currentPage + 1; i <= totalPages; i++) {
          ...
          selectedJobCdrList = selectedJobCdrList.concat(results);
          if (selectedJobCdrList.length >= 5000) break;
        }
      }
  ...
  async fetchCompareGeoFenceData() {
    ...
    if (selectedCaseLocationJobCdrList.length === 2) {
      ...
      try {
        response = await getCdrComparision(
